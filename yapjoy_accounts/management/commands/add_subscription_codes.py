...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        with open('B_G_List.csv', 'rU') as f:
            ...
            bl = 0
            a = None
            data = [['email', 'name', 'code'],]
            with open('static/output.csv', 'wb') as fp:
                a = csv.writer(fp, delimiter=',')
            for row in reader:
                if re.match("([^@|\s]+@[^@]+\.[^@|\s]+)",row[1]):
                    try:
                        user = User.objects.get(email=row[1])
                        subcode = SubscriptionCode.objects.get_or_create(user=user)[0]
                        print 'done'


                        data.append([user.email,user.get_full_name(),subcode.code])
